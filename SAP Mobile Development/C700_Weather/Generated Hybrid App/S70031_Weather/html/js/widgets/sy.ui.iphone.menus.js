...
$.widget("sy.actionSheet", {
          ...
          this.element.bind('click', function() {
            ...
            if (!  self.showPage ){
           	  $( document ).one( "vclick",function( ){
         			  if ( self._clickedOnScreen()) {
          				  //self.cancelButton.touchend();	// NA12-2148 - This didn't work when generating from MobilSDK
						  self.close();						// but this did. Backwards compatible generation worked with the touchend().
          			  }
           	  });
            }     
    ...
    _clickedOnScreen:function() {
    	var parent =  event.target.parentNode;
    	while(  parent !== null && parent.nodeName !== 'BODY') {
    		if ( parent.getAttribute('id')  !== 'as-wrapper') {
    ...
    open:function() {
        ...
        // alert( "from open start to created all the menus cost="+ t2 + " , will open the page");
         if ( self.actionDlg.height() > $(window).height() -15 ) {
             ...
             $('div[data-role="content"]', actionDlg ).scrollview({isDialog : true, maxScrollYAdjust: 96 });
             $.mobile.changePage(actionDlg,  { transition:'pop', reverse:false, changeHash: false }); 
              ...
              var newtop =  window.innerHeight  + 240 - actionDlg.height() +'px';; //why 250; it works , always keeps it start from bottom.
