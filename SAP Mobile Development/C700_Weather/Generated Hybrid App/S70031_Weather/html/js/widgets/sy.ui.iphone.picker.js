...
$.widget("sy.valuePicker", {
   ...
   _refreshSlots:function() {
    ...
    open: function () {
        ...
        var initScroll = $(this.element).offset().top;
        ...
        this.swWrapper.style.webkitTransitionDuration = hwc.isAndroid() ? '200ms' : '400ms';
        ...
        //jira issue #NA12-2532
        window.setTimeout(function(){
            self.screen.bind('click', function() {
                self.cancelAction();
                self.close();
           });
        }, 200);
    ...
    _createSlots: function () {
        ...
        this.swHeader = document.getElementById('sw-header');                   // Header band w/buttons
        ...
        this.swHeader.addEventListener('mousemove', this, false);       // Prevent page scrolling
        this.swHeader.addEventListener('mousedown', this, false);       // Prevent page scrolling
    },
    ...
    destroy: function () {
        ...
        this.swHeader.removeEventListener('mousemove', this, false);
        this.swHeader.removeEventListener('mousedown', this, false);
    ...
    handleEvent: function (e) {
        ...
        if (e.currentTarget.id == 'sw-header') {
            this._lockScreen(e);
        }
...
var DatePicker =  $.extend({}, $.sy.valuePicker.prototype, 
          (function(orig) {
               ...
               _init: function() {
                   ...
                   if ( self.options.onCancel != null ) {
                      this.cancelAction = self.options.onCancel;
                        ...
                        this.cancelAction = function (){
                            if (hwc.isLocaleDatetimeFormat(this.element[0]) && !(hwc.isIOS6() || hwc.isIOS5())) {
                                var cType = getAttribute(this.element[0], "sup_html_type");
                                if (cType === "time") {
                                    this.element[0].value = hwc.getLocaleTimeString(new Date(hwc.parseTime(this.element[0].ISODateTimeValue)));
                                }
                                else if (cType === "date") {
                                    this.element[0].value = hwc.getLocaleDateString(new Date(hwc.parseDateTime(this.element[0], this.element[0].ISODateTimeValue)));
                                }
                                else if (cType === "datetime-local") {
                                    this.element[0].value = hwc.getLocaleDateTimeString(new Date(hwc.parseDateTime(this.element[0], this.element[0].ISODateTimeValue)));
                                }
                            }
                            else
                            {
                                this.element[0].value = this.element[0].ISODateTimeValue;
                            }
                        };
                   }
                   if ( self.options.onDone != null ) {
                        ...
                        this.doneAction = function( value ){
                            ...
                            var allowsNull = $(this.element[0]).attr("sup_allows_null");
                            if (allowsNull != null && allowsNull != undefined) {
                                $(this.element[0]).attr("sup_valuechanged", 'true');  
                            }
                        };
                   }
                   ...
                   this.clickHandler = function( ) {
                       ...
                       if ( hwc.isIOS()) {
                           //  Make sure the datepicker doesn't popup when the control is readonly (attribute is 'disabled')
                           var test = getAttribute(self.element[0], "disabled");
                           if (test != null && test != undefined && test == 'disabled') {
                               return;
                           }
                       }
               ...
               if (hwc.isIOS()){
                       this.element.bind('touchend', this.clickHandler);
                  }else {
               ...
               _createDateSlots: function() {
                   ...
                   if(isLocalizedDisplay == "false" || isLocalizedDisplay == undefined || isLocalizedDisplay == null || hwc.isIOS5()) {
                   ...
                   if ( defaultValue  != null && defaultValue != ""){
                       ...
                       defDate.push( now.getFullYear().toString());
                       defDate.push((now.getMonth() + 1).toString());    // month is 0 based.
                       defDate.push(now.getDate().toString());
                        this.separator = "-"; 
                   } 
...
var TimePicker =  $.extend({}, $.sy.datePicker.prototype, 
          (function(orig) {
               ...
               _createTimeSlots:function() {
                   ...
                   // Null value support
                   if (defaultValue == null || defaultValue == "") {
                       defaultValue = now.getHours().toString() + ":" + now.getMinutes().toString() + ":" + now.getSeconds.toString();
                   }                    
