...
//#NA12-2596, when jqm handles touchevens which causes delay for native context menu popup.( link for tel:)
//If it's phonelink , in handleTouchStart and handleTouchEnd ,doing nothing fixed the issue.

function isPhoneLink( event ){
	return event.target.tagName === 'A' && event.target.href.indexOf('tel:') >= 0
}
...
function handleTouchStart( event ) {
	...
	if ( touches && touches.length === 1 && !isPhoneLink(event) ) {
...
function handleTouchEnd( event ) {
	if ( blockTouchTriggers || isPhoneLink( event)) {
		return;
	}
			...
			if ( eventCaptureSupported ) {
				...
				if (activeDocHandlers[ "touchstart" ] === 1) {
					$document.bind( "touchstart", handleTouchStart )
						...
						//#NA12-2596, iscroll handle the scroll, we shouldn't do this here.
...
(function( $, window ) {
	...
	$.event.special.orientationchange.orientation = get_orientation = function() {
		...
		//CR#698818-1, window.orientation is always same no matter you are in landscape or portrait,
		//The fix was use  the code in else block which is the older way JQM ( in 1.0a4 version ) detect the orientation.
		if ( false ) { // $.support.orientation ) {
...
$.widget( "mobile.dialog", $.mobile.widget, {
	...
	close: function() {
		var pre =  $.mobile.urlHistory.getPrev();
		if ( pre != null && ! $.mobile.changePage.defaults.changeHash  ) {
		    $.mobile.changePage( $('#'+pre.pageUrl),{
					transition:'slide',
					reverse:true,
					changeHash: false });//this setting won't add page id to the url.
		}else {
		    window.history.back();
		}
	}
...
$.widget( "mobile.textinput", $.mobile.widget, {
	...
	_create: function() {
		...
		// Autogrow
		/*CR#666333-1, CR#672941-1*/
		if ( input.is('textarea[readonly="readonly"]') && (navigator.userAgent.indexOf("Android") > -1) ) {
	...
	_create: function() {
		...
		this.handle.bind({
			...
			vmousedown: function() {
			    ...
			    //CR#https://supjira.sybase.com:8443/browse/NA12-1021
			    //Note: I think this only happens for iOs 4.2 or less, and for Android, it happens for 2.3 or less
			    //Tested with iOS 5 and Android 4.0 , both worked fine, bot not iOS 4.2 and Android 2.3
			    if ((navigator.platform.indexOf("i") === 0) 
					||(navigator.userAgent.indexOf("Android") > -1) ) {
				    //$( this ).focus();, caused the screen blank, this is the af·fect of combination of using iscroll and JQM
			    }else  {
				    $( this ).focus();
			    }
		    },
...
$.widget( "mobile.selectmenu", $.mobile.widget, {
	...
	_focusButton : function() {
		...
		setTimeout( function() {
			...
			//CR#675381-1,  self.button.focus() causes the page moves way up leaving empty space on the screen  
			//in the case of when selection on the bottom part of the page and invisible at initial stage,
			//thenyou need to scroll up and make it visible. 
			//Note: I think this only happens for iOs 4.2 or less, and for Android, it happens for 2.3 or less
			if ((navigator.platform.indexOf("i") === 0) ||(navigator.userAgent.indexOf("Android") > -1) ) {
				self.refresh();
			}else{
				self.button.focus();
			}
  ...
  _selectOptions: function() {
	...
	_preExtension: function(){
		...
		// CR#689144-2, jQueryMobile searched label from whole html pages, in our case, it should only be search from the page the choices located.
		//this.label = $( "label[for='"+ this.selectID +"']" ).addClass( "ui-select" );
		this.label = this.element.closest( "form,fieldset,:jqmData(role='page')" ).find( "label" ).filter( "[for='" + this.element[ 0 ].id + "']").addClass( "ui-select" ),
	
		//jira#3120,when the label clicked, the choice button gets forcus which caused the screen move up about 20px, remove "for" attribute , fixed the issue.
		this.label.attr("for", "");
		...
		$.extend( widget, {
			...
			build: function() {
				...
				self.list.attr( "role", "listbox" )
					...
					.delegate( "li:not(.ui-disabled, .ui-li-divider)", "click", function( event ) {
						...
						//<<--- CR# , no cr number, during upgrading test, found the issue which was you have to clicked two to close the listview
						//clicked twice the drop-down button for open it on ios device. fixed it by following code
						if ( ! self.isOpen ) {
							event.preventDefault();
							return;
						}
			...
			close: function() {
				...
				if ( self.menuType == "page" ) {
				   ...
				   //CR# , no cr number for this. if we use window.history.back(), it would go to previous link's page , in our workflow case, it would be blank page.
				  $.mobile.changePage(self.thisPage,{
			...
			open: function() {
                ...
                //add active class to button
				self.button.addClass( $.mobile.activeBtnClass ); //move a little bit up, this may set the active button first. this may give feedback to user quicker.
				...
				if ( menuHeight > screenHeight - 80 || !$.support.scrollTop ) {
					...
					self.menuPageClose.click(function(){
						self.close();
					});
					
					self.menuPage.one( "pageshow", function() {
						
						//CR#689886-2, the item user seleted in choice control cannot be highlight
						//When drop-down showinng as page, it only set active button during init time, not afterwards. 
						//fixed it by looking for selected to true and set the active button calss.
						var items = self.list.children();
						for( var i =0 ; i < items.length ; i++ ){
							 if ( items[i].getAttribute('aria-selected')  === 'true') {
								 $(items[i]).addClass( $.mobile.activeBtnClass );
								 break ; //TODO: fixme: if it's multi selection.
							 }
						}
				
						//<<----CR# no number, it caused weird behavior for on iOS --->
						if ( ! isIOS() )
						{
						    focusMenuItem();
						}
