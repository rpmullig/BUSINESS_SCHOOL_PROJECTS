...
function menuItemCallbackStartStartEmployeesMenuItemKey() {
    if (!hwc.customBeforeMenuItemClick('Start', 'StartEmployeesMenuItemKey')) {
        return;
    }
    var rmiKeys = [];
    var rmiKeyTypes = [];
    ...
    var dataMessageToSend = hwc.getMessageValueCollectionForOnlineRequest('Start', 'StartEmployeesMenuItemKey', rmiKeys, rmiKeyTypes);
    var inputOnlyDataMessageToSend = hwc.getMessageValueCollectionForOnlineRequest('Start', 'StartEmployeesMenuItemKey', rmiInputOnlyKeys, rmiInputOnlyKeyTypes);
    if (hwc.validateScreen('Start', hwc.getCurrentMessageValueCollection(), rmiKeys) && 
        hwc.saveScreens(true)) {
        hwc.doOnlineRequest('Start', 'StartEmployeesMenuItemKey', 60, 0, '', null, dataMessageToSend, inputOnlyDataMessageToSend.serializeToString());
    }
    hwc.customAfterMenuItemClick('Start', 'StartEmployeesMenuItemKey');
}


function menuItemCallbackEmployeeDetailBack() {
    if (!hwc.customBeforeMenuItemClick('EmployeeDetail', 'Back')) {
        return;
    }
    doSaveAction();
    hwc.customAfterMenuItemClick('EmployeeDetail', 'Back');
}
function menuItemCallbackEmployeeDetailCancel() {
    if (!hwc.customBeforeMenuItemClick('EmployeeDetail', 'Cancel')) {
        return;
    }
    doCancelAction();
    hwc.customAfterMenuItemClick('EmployeeDetail', 'Cancel');
}


function menuItemCallbackEmployeeSubmit() {
    if (!hwc.customBeforeMenuItemClick('Employee', 'Submit')) {
        return;
    }

    if (hwc.saveScreens()) {
        hwc.doSubmitWorkflow('Employee', 'Submit', '', '');
    }
    hwc.customAfterMenuItemClick('Employee', 'Submit');
}


function menuItemCallbackEmployeeCancel() {
    if (!hwc.customBeforeMenuItemClick('Employee', 'Cancel')) {
        return;
    }
    doCancelAction();
    hwc.customAfterMenuItemClick('Employee', 'Cancel');
}
