...
$.widget("sy.menuBar", {
	_create: function(){
		...
		var self = this;
                ...
                $(window).bind("throttledresize",function(){
                    if ( self._isThisScreen($.mobile.activePage.attr('id')) ){
	...
	_reArrangeBttons:function( buttons, isActionSheetBtAtCenter ){
		var screen = document.getElementById(this.screenName);
        if (getAttribute(screen, "sup_autoarrangemenus") === 'false') {
            return;
        }
		// We have to clone first and then remove and then add the again
		var newButtons = [];
			...
			return {
				positionButtons:function() {
					...
					buttons = this.element.find('a');
                                        ...
                                        if ( buttons.length == 0) {
                                            if ( title.length > 0 && 	title.width() < ( this.element.width() - 20 ) ) {
                                                var leftMg =  ( this.element.width() - 20 )/2 - title.width()/2 ;
                                                title.css("margin-left",leftMg+'px');
                                            }
                                            return ;
                                        }
					...
					if (rightbt&& rightbt.length > 0 ) {
						...
						if (emptySpace > 0 ) {   //we have more space on the bar.
							...
							//when we don't have enough space, reduce left button text padding, so we could at least show one letters instead of showing "..."
							//Notes: on devices, we still could reduce to 16px which would show first two letters, but on simulator, for 16px, the first letter of the menu would overlap with left-arrow icon.
							this.element.find( '.ui-btn-icon-left').find('.ui-btn-inner').css('padding-left', '22px');
						} //end of emptySpace < 0 
						...
						if ( emptySpace < 0 ) {  //squeeze title and left button.
							...
							var tl =  parseInt( defTitleW ) +parseInt( half + adjust );
							title.css("width", tl+ "px")
...
//issue #1, when on the footer bar only have one button, we don't generate  class="ui-btn-left" , so i  line 229, btleft = null, but we do have left button.
		...
		(function(orig) {
			return {
				positionButtons:function() {
					...
					for( var i =0; i < hiddentbs.length; i++ ) {
						this._makeButton( hiddentbs[i]);
					}
					...
					var maxButtonsOnBar;
			        if (getAttribute(screen, "sup_autoarrangemenus") === 'false') {
			            return; //we don't do any re-positioning for the buttons
			        }
			        else
			        {
			        	maxButtonsOnBar = 3;
			        }
					if ( (bts.length + hiddentbs.length ) > maxButtonsOnBar ) {
						...
						if (needRepositonButtons ){
							...
							if ( bts.length === 0 ) {  //no visible button on the bar, remove two menu from actionsheet to footer bar , 
								...
								//If we have one visible, we need two invisible to become visible, j < bts.length is wrong since bts.length =1 at this case,
								//fix: 
							//	for( var j = 0; j < hiddentbs.length && j < bts.length ; j++) {
								for( var j = 0; j < hiddentbs.length  ; j++) {
									
								    $(hiddentbs[j]).attr('data-role', 'button');
									// NA12-2111 add ui-footer instead of ui-btn-left etc.
									if (! btleft ) { //if we already have left , put it into right  
										$(hiddentbs[j]).removeClass('invisible_button ui-btn-right ui-btn-center').addClass('ui-footer');
										btleft = hiddentbs[j];
									}else  if ( ! btcenter) {
										$(hiddentbs[j]).removeClass('invisible_button ui-btn-left ui-btn-right').addClass('ui-footer');
										btcenter = hiddentbs[j];
									}else if (! btright ){
										$(hiddentbs[j]).removeClass('invisible_button ui-btn-right ui-btn-center').addClass('ui-footer');
										btright= hiddentbs[j];
									}
									if ( j + bts.length +1> maxButtonsOnBar ) {
										break;
									}
								}
							}
							...
							// NA12-2111 - rearranging scrambles the footer.
						...
						if ( needRepositonButtons) {
							...
							if ( bts.length === 0 ) { //if we have zero visible button, move menus from actionsheet to the bar.
								...
								// NA12-2111 Get a reasonable width before changing role and class.
								$(hiddentbs[0]).css('width', 'auto');
								var width = $(hiddentbs[0]).width();								
								$(hiddentbs[0]).attr('data-role', 'button');

								if (  !btleft ) { //if we already have left , move to right 
									// NA12-2111 ui-btn-left, ui-btn-right have no effect in the footer.
									$(hiddentbs[0]).removeClass('invisible_button ui-btn-right ui-btn-center'); //.addClass('ui-btn-left');
								}else{
									$(hiddentbs[0]).removeClass('invisible_button ui-btn-left ui-btn-center'); //.addClass('ui-btn-right');
								}
								$(hiddentbs[0]).css("width" , parseInt(width) + 'px'); // NA12-2111 set the reasonable width.
							}
						}
					}
					...
					// NA12-2111 add the ui-footer class to all the footer buttons.
					for (var i = 0; i < bts.length; i++) {
						$(bts[i]).addClass('ui-footer');
					}
